...
(Two datasets are first-party, LLM-generated, and *are* MIT — see **First-party generated data** at the end.)
...
## First-party generated data (spec 005) — MIT

Two datasets are **generated by HanjaKey itself** (an LLM subagent swarm), not sourced from a third party,
so they are **our own data under the project `LICENSE` (MIT)** — listed here only for provenance. They are
LLM-generated short factual labels (tiers, 2–4-word 훈음), not copied from any source's prose, so they carry
only thin copyright and stay MIT. Pipeline + provenance: `docs/specs/005-candidate-quality/`.

| Generated data | Bundled as | License | Used for |
|---|---|---|---|
| Swarm preference tiers | `Sources/HanjaKitCore/Resources/data/curation-swarm/tiers.txt` | MIT (first-party) | single-syllable candidate ordering (spec 005 M2): 26,988 rows |
| Swarm single-Hanja gloss | `Sources/HanjaKitCore/Resources/data/hanja-gloss-swarm/hanja_gloss.txt` | MIT (first-party) | 훈음 fill for ranked tier 0–2 + unranked, no clean-licensed source (spec 005 ②): 1,586 entries |

- `tiers.txt` (`읽기:한자:티어:글로스`, file-ordered per reading by `(tier, rank)`) built by
  `scripts/build_tiers.py`, hardened against swarm glitches (validate vs the canonical `hanja.txt`, drop
